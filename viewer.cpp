...
static GLfloat  dout = 100.0; // 100.0; // initial focus
...
int     n = cell->kids;
        if (n==0) return;
        DO(i,n) {
                DO(j,3) scene->rank[j][total] = scene->rank[j][total+Cell::total] = cell->child[i];
		total++;
        }
        DO(i,n) sceneCell(cell->child[i], scene);
}
...
Cell	*scene;
	scene = new Cell;
        ...
        DO(j,3) { scene->rank[j] = new Cell*[n]; TEST(scene->rank[j]) }
	total = 0;
...
Vec	x,y,z;
	...
	z = (x^y).norm(); y = (z^x).norm();	// orthogonalise + normalise
...
int	thick = depth-level;
	...
	// bondlink == 0  = default N--C link
	if (bondlink < 0) bondlink = -bondlink;
...
float	cthru, range = 0.00001,
	...
	if (hide < 0) range *= -hide; // becomes solid closer
        ...
        cent.x = focus[0]; cent.y = focus[1]; cent.z = focus[2];
        aa = cent||cell->xyz;
        bb =  eye||cell->xyz;
        cc =  eye||cent;
...
float	cthru;
	...
	if (colours==2) { // flash bumps (intra=yellow, inter=magenta)
		if (hit) {
			if (parent==hit->parent) col = 5; else col = 4;
		}
	}
...
float	d;
	...
	gluLookAt(eye.x, eye.y, eye.z, focus[0],focus[1],focus[2], View.B.x, View.B.y, View.B.z);
	rankLive();
