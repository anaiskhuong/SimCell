...
#define N 12	// size of model data array (N cols per row = levels))
...
public:
	...
	float	rejel[N];	//  kick strength for soft repulsion (jelly)
				//	-ve don't repel children within parent 
	float	adh[N];		//  strength of adhesion
	float	cil[N];		//  strength of CIL
	...
	static	int	norun, noview, putpdb;
	static	int	frame, hidden, tinker;
};

extern char  *names;
extern int   *shape, *links, *chain, *split, *local, *align;
extern float *sizes, *bumps, *kicks, *keeps, *bonds, *repel, *rejel, *adh, *cil;
