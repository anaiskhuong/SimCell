...
int	moltype = param->moltype,
	...
	if (protein && level==depth-1 && cell->type==2) { // protein SSE
		if (sort==2) { // beta (axis set line by i-1,i average)
			in = 0;
			axis.zero();
			was = (cell->child[1]->xyz + cell->child[0]->xyz)/2;
			for (i=1; i<n; i++) { Cell **kidi = cell->child;
		...
		if (sort==1) { // alpha (axis set line by i-1,i,i+1 average)
			in = 0;
			axis.zero();
			was = (cell->child[2]->xyz + cell->child[1]->xyz + cell->child[0]->xyz)/3;
			for (i=1; i<n-1; i++) { Cell **kidi = cell->child;
	...
	DO(i,n) fixSSEaxis(cell->child[i], weight);
}
